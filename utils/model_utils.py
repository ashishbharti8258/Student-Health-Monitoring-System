...
# BUG FIX: joblib.load() was being called fresh on every single rerun of
# every page (Streamlit reruns the whole script on each interaction), which
# is slow and pointless since the artifacts never change at runtime.
# st.cache_resource keeps one shared, in-memory copy across reruns/pages.
...
def transform(df_raw: pd.DataFrame, prep: dict) -> pd.DataFrame:
    ...
    columns_before_encoding = set(df.columns)
...
def get_feature_importance(model, prep: dict, top_n: int = 8) -> pd.DataFrame:
    """Return the model's top-N most important features as a tidy DataFrame."""
    feature_columns = prep["feature_columns"]
